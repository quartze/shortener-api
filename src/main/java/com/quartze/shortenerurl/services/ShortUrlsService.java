...
public class ShortUrlsService {
    ...
    public ShortUrls createNewShortsUrl(String originalUrl, Boolean isNsfw, String token) {
        ...
        if(token != null) {
            Long user_id = AuthUtils.getIdFromToken(token);
    ...
    public void deleteShortUrl(String id, String token) {
    ...
    public Page<ShortUrls> getShortUrlsByUser(String token, PageRequest page) {
        long user_id = AuthUtils.getIdFromToken(token);
        User user = userService.getUser(user_id);
        if(user == null) throw new TokenIsInvalidException();
